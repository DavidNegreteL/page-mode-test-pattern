...
class CheckoutPage {
  constructor() {
    ...
    this.nextShippingBtn = Selector(
      "input.button-1.shipping-method-next-step-button"
    );
    this.nextPaymentBtn = Selector(
      "input.button-1.payment-method-next-step-button"
    );
    this.nextConfirmBtn = Selector(
      "input.button-1.payment-info-next-step-button"
    );
    this.confirmOrderBtn = Selector(
      "input.button-1.confirm-order-next-step-button"
    );
    this.orderConfirmationMessage = Selector("strong").withText(
      "Your order has been successfully processed!"
    );
    this.viewOrderDetailsLink = Selector("a").withText(
  ...
  async selectCountry(country) {
    ...
    await t.click(this.countryList).click(countryOption.withText(country));
  }
}
