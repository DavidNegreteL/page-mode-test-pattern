...
test("Place Order E2E Tests", async (t) => {
  await t
    ...
    .navigateTo(
      "https://demo.nopcommerce.com/register?returnUrl=%2F%3FbypassCaptcha%3Dtrue"
    )
    .expect(getURL())
    .contains("register")
  ...
  await t
    ...
    .expect(productdetails.productPrice.exists)
    .ok()
    .selectText(productdetails.prductQuantity)
    .pressKey("delete")
    .typeText(productdetails.prductQuantity, "3")
    .click(productdetails.addToCart)
    .expect(productdetails.successMessage.exists)
    .ok()
    .wait(3000)
  ...
  await t
    ...
    .expect(checkoutpage.orderConfirmationMessage.exists)
    .ok()
